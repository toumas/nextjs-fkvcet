// From: https://www.npmjs.com/package/airconsole-typescript
declare global {
  ...
  class AirConsole {
    ...
    onDeviceStateChange(
      device_id: number,
      user_data: AirConsoleStates.DeviceState
    ): void;
     ...
     */
    getProfilePicture(
      device_id_or_uid?: number | string,
      size?: number
    ): string | undefined;
    ...
    // from: https://github.com/AirConsole/airconsole-events/
    /**
     * Returns eventId for .off
     */
    on: <T>(
      eventName: string,
      cb: (device_id: number, params: T) => void,
      context?: any
    ) => number;
    ...
    dispatchEvent: <T>(
      device_id: number,
      data: { event_name: string; params?: T }
    ) => void;
     ...
     */
    observeCustomProperty: <T>(
      device_id: number,
      property: string,
      callback: (new_value: T, old_value: T) => void
    ) => void;
  }
}

type orientation =
  | AirConsoleConstants.ORIENTATION_LANDSCAPE
  | AirConsoleConstants.ORIENTATION_PORTRAIT;
